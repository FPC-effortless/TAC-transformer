...
def _config_for_variant(variant: str) -> TACConfig:
    return TACConfig(
        ...
        memory_system_type=(
            "multi_timescale" if variant == "multi_timescale" else "flat"
        ),
...
def _evaluate_variant(
    ...
    *,
    ...
    for _ in range(eval_batches):
        ...
        probs = F.softmax(carry_logits, dim=-1)
        max_prob, _ = probs.max(dim=-1)
        should_accept = (carry_pred != UNKNOWN) & (max_prob >= confidence_threshold)
        ...
        veto_unknown_hallucinations += int((should_accept & unknown).sum())
        accepted += int(should_accept.sum())
        total += int(answer.numel())

    return {
        "carry_accuracy": carry_known_correct / max(known_total, 1),
        "reset_accuracy": reset_known_correct / max(known_total, 1),
        "semantic_procedural_ablation_accuracy": ablated_known_correct / max(
            known_total,
            1,
        ),
        ...
        / max(unknown_total, 1),
        "veto_unknown_hallucination_rate": veto_unknown_hallucinations
        / max(unknown_total, 1),
        "veto_coverage": accepted / max(total, 1),
    }
...
def run_experimental_memory_validation(
    *,
    output_dir: Path,
    ...
    multi = variants["multi_timescale"]
    flat = variants["flat_control"]
    validation_passed = (
        multi["carry_accuracy"] >= flat["carry_accuracy"] + 0.05
        and multi["carry_accuracy"] >= multi["reset_accuracy"] + 0.05
        and multi["semantic_procedural_ablation_accuracy"]
        <= multi["carry_accuracy"] - 0.02
        and multi["veto_unknown_hallucination_rate"]
        <= multi["forced_unknown_hallucination_rate"]
    )
    result = {
        "method": {
            "experiment_type": "actual_tac_training",
            "hypotheses": [
                "Multi-timescale IdentityState improves carried known-query accuracy over flat memory and reset controls.",
                "Semantic/procedural tier ablation causally degrades carried-query accuracy.",
                "An explicit unknown/veto policy reduces accepted hallucinations on unsupported queries.",
            ],
            "controls": [
                "flat_control",
                "reset_identity_state",
                "semantic_procedural_ablation",
                "forced_answer_without_veto",
            ],
            "train_steps": train_steps,
            "eval_batches": eval_batches,
            "batch_size": batch_size,
        ...
        "decision": {
            ...
            "boundary": "Actual small TAC training on a synthetic support-query memory task; not an external checkpoint or broad agent benchmark.",
        },
    }
    artifact_path = output_dir / "memory_energy_experimental_validation.json"
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("runs/benchmarks/memory_energy_experimental_tac221_2026_06_10"),
    )
