...
from tac_transformer.evaluation import benchmark_effectiveness


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run TAC effectiveness probes against a vanilla baseline."
    )
    ...
    parser.add_argument("--short-loss-tolerance", type=float, default=1.05)
    parser.add_argument("--min-memory-delta", type=float, default=1e-4)
    parser.add_argument("--min-shuffle-penalty", type=float, default=1e-4)
...
def main() -> None:
    ...
    result = benchmark_effectiveness(
        ...
        min_memory_delta=args.min_memory_delta,
        min_shuffle_penalty=args.min_shuffle_penalty,
    )
